...
public class LevelUpScreen : MonoBehaviour
{
    ...
    public void ShowChoices(GobboController gobbo)
    {
        ...
        currentGobbo = gobbo;
        currentContext = gobbo != null && gobbo.NeedsEvolutionChoice()
            ? GobboCardContext.EvolutionChoice
            : GobboCardContext.RunLevelUp;
    ...
    void BuildChoices()
    {
        ...
        // If the reroll exclusion pool got too strict, allow repeats instead of giving no cards.
        ...
        {
            if (card != null && !seenThisSelection.Contains(card.cardId))
                seenThisSelection.Add(card.cardId);
        }
        ...
        for (int i = 0; i < buttons.Length; i++)
        {
            ...
            if (buttons[i].text != null)
                buttons[i].text.text = card.cardName + "\n" + card.description;
    ...
    {
        if (currentGobbo == null || card == null)
        {
            Hide();
            return;
        }
    ...
    void TryReroll()
    {
        if (currentGobbo == null || GameState.Instance == null)
            return;
    ...
    {
        int shinies = GameState.Instance != null ? GameState.Instance.gobbo.shinies : 0;

        if (rerollText != null)
            rerollText.text = "Reroll (" + rerollCost + " shiny)";

        if (rerollButton != null)
            rerollButton.interactable = shinies >= rerollCost && currentChoices.Count > 0;
    }
