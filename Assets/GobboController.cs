...
public class GobboController : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        if (buddyRoster == null) buddyRoster = Object.FindAnyObjectByType<BuddyRoster>();
    ...
    void UpdateDirectionalSprite()
    {
        ...
        if (dir.y > 0.35f)
        {
            if (dir.x < -0.35f && backLeft != null) spriteRenderer.sprite = backLeft;
            else if (dir.x > 0.35f && backRight != null) spriteRenderer.sprite = backRight;
            else if (back != null) spriteRenderer.sprite = back;
        }
        else
        {
            if (dir.x < -0.35f && frontLeft != null) spriteRenderer.sprite = frontLeft;
            else if (dir.x > 0.35f && frontRight != null) spriteRenderer.sprite = frontRight;
            else if (front != null) spriteRenderer.sprite = front;
        }
    }
    ...
    void UpdateTimers()
    {
        ...
        if (roarCooldownTimer > 0f) roarCooldownTimer -= Time.deltaTime;
        if (sporeMendCooldownTimer > 0f) sporeMendCooldownTimer -= Time.deltaTime;
        if (dashBiteCooldownTimer > 0f) dashBiteCooldownTimer -= Time.deltaTime;
        if (isDashing)
        {
    ...
    void Move()
    {
        ...
        Vector2 desiredVelocity = isDashing ? aimDirection * dashSpeed : moveInput * moveSpeed;
        if (visualController != null) visualController.SetAnimationState(isDashing ? GobboAnimationState.Dash : (moveInput.sqrMagnitude > 0.01f ? GobboAnimationState.Walk : GobboAnimationState.Idle));
        TileMover.Move(rb, desiredVelocity, bodyRadius);
    }

    void HandleActions()
    {
        if (Input.GetMouseButton(0)) TryDig(); else digTimer = 0f;
        if (Input.GetMouseButtonDown(1)) BasicAttack();
        if (Input.GetMouseButtonDown(2)) TryDashBite();
        if (Input.GetKeyDown(KeyCode.LeftShift)) TryDash();
        if (Input.GetKeyDown(KeyCode.E)) Interact();
        if (Input.GetKeyDown(KeyCode.Q)) PlaceSpore();
        if (Input.GetKeyDown(KeyCode.F)) ToggleFollowersFollow();
        if (Input.GetKeyDown(KeyCode.C)) ToggleFollowerCombat();
        if (Input.GetKeyDown(KeyCode.Space)) Roar();
        if (Input.GetKeyDown(KeyCode.R)) SpecialAbility();
    }
    ...
    void Dig()
    {
        if (visualController != null) visualController.SetAnimationState(GobboAnimationState.Dig);
        Vector2 digPoint = (Vector2)transform.position + aimDirection.normalized * digRange;
        if (MapGenerator.Instance != null) MapGenerator.Instance.DigCircle(digPoint, digRadius);
        Collider2D[] hits = diggableLayers.value == 0 ? Physics2D.OverlapCircleAll(digPoint, digRadius) : Physics2D.OverlapCircleAll(digPoint, digRadius, diggableLayers);
        foreach (Collider2D hit in hits)
        {
            RevealCover revealCover = hit.GetComponent<RevealCover>();
            if (revealCover != null) revealCover.Dig(digPower);
        }
    }
    ...
    void BasicAttack()
    {
        ...
        if (visualController != null) visualController.SetAnimationState(GobboAnimationState.Attack);
        ...
        Collider2D[] hits = enemyLayers.value == 0 ? Physics2D.OverlapCircleAll(attackPoint, attackRadius) : Physics2D.OverlapCircleAll(attackPoint, attackRadius, enemyLayers);
        int damage = CalculateAttackDamage(1f);
        int hitCount = 0;
        foreach (Collider2D hit in hits)
        {
            if (hit.gameObject == gameObject) continue;
            if (!MapPathfinder.HasLineOfWalkableSight(transform.position, hit.transform.position)) continue;
    ...
    void TryDashBite()
    {
        if (!hasDashBite || dashBiteCooldownTimer > 0f) return;
        Transform target = GetEnemyUnderCursor();
        if (target == null) return;
        float distance = Vector2.Distance(transform.position, target.position);
        if (distance > dashBiteRange) return;
        if (!MapPathfinder.HasLineOfWalkableSight(transform.position, target.position)) return;
        ...
        if (MapGenerator.Instance == null || MapGenerator.Instance.IsWorldPositionClearForBody(desiredPosition, bodyRadius)) rb.position = desiredPosition;
    ...
    Transform GetEnemyUnderCursor()
    {
        ...
        Collider2D hit = enemyLayers.value == 0 ? Physics2D.OverlapPoint(point) : Physics2D.OverlapPoint(point, enemyLayers);
    ...
    {
        if (target == null) return;
        BuddyCombat[] buddies = Object.FindObjectsByType<BuddyCombat>(FindObjectsSortMode.None);
        foreach (BuddyCombat buddy in buddies)
            if (buddy != null) buddy.SetTarget(target);
    }
    ...
    void Interact()
    {
        ...
        foreach (Collider2D hit in hits)
        {
            ...
            if (campInteractable == null) continue;
    ...
    void PlaceSpore()
    {
        ...
        bool hasSpore = false;
        if (sporeInventory != null) hasSpore = sporeInventory.UseSpore();
        else if (sporeCount > 0) { sporeCount--; hasSpore = true; }
        if (!hasSpore)
        {
            Debug.Log("No spores.");
            return;
        ...
        if (grow != null && grow.buddyPrefab == null) grow.buddyPrefab = buddyPrefab;
    }

    public void SpawnBuddy()
    {
        if (buddyRoster == null) buddyRoster = Object.FindAnyObjectByType<BuddyRoster>();
        GobboUnitSaveData data;
        if (buddyRoster != null)
        {
            data = buddyRoster.CreateNewBuddy();
            if (GameState.Instance != null) GameState.Instance.RegisterGobboFound(data);
        }
        else
        {
            Debug.LogWarning("No BuddyRoster found. Spawning unsaved gobbo.");
            data = new GobboUnitSaveData();
            BuddyProgression.PrepareNewBaby(data);
        }
        Vector2 spawnPos = (Vector2)transform.position + Random.insideUnitCircle * buddySpawnRadius;
        SpawnGobboUnit(data, spawnPos);
    }

    public void SpawnGobboUnit(GobboUnitSaveData data)
    {
        Vector2 spawnPos = (Vector2)transform.position + Random.insideUnitCircle * buddySpawnRadius;
        SpawnGobboUnit(data, spawnPos);
    }

    public void SpawnGobboUnit(GobboUnitSaveData data, Vector2 spawnPosition)
    {
        if (buddyPrefab == null)
        {
        ...
        if (data == null)
        {
            Debug.LogWarning("Tried to spawn gobbo with no unit data.");
            return;
        }
        ...
        else Debug.LogWarning("Buddy prefab is missing BuddyUnit.");
    ...
    {
        BuddyFollow[] follows = Object.FindObjectsByType<BuddyFollow>(FindObjectsSortMode.None);
        foreach (BuddyFollow follow in follows) if (follow != null) follow.enabled = followersFollowing;
        BuddyCombat[] combats = Object.FindObjectsByType<BuddyCombat>(FindObjectsSortMode.None);
        foreach (BuddyCombat combat in combats) if (combat != null) combat.enabled = followersAggressive;
    }

    void Roar()
    {
        if (roarCooldownTimer > 0f) return;
        roarCooldownTimer = roarCooldown;
        Debug.Log("Gobbo roar: " + roarType);
        if (roarAudio != null) roarAudio.Play();
    }
    ...
    void TrySporeMend()
    {
        if (sporeMendCooldownTimer > 0f) return;
    ...
    {
        AddXP(value);
        if (foodValue > 0 && GameState.Instance != null) GameState.Instance.RegisterFoodValueGained(foodValue);
    ...
    {
        if (amount <= 0) return;
        if (GameState.Instance != null) GameState.Instance.RegisterXPGained(amount);
        xp += amount;
    ...
    void LevelUp()
    {
        ...
        levelUpScreen = Object.FindAnyObjectByType<LevelUpScreen>(FindObjectsInactive.Include);
        if (levelUpScreen != null) levelUpScreen.ShowChoices(this);
        else Debug.LogWarning("No LevelUpScreen found in scene.");
        UpdateSize();
    }
    ...
    public bool NeedsEvolutionChoice() => pendingEvolution || (level == 2 && gobboType == BuddyType.Baby);
    public void ApplyCard(GobboCard card) { if (card != null) card.ApplyToPlayer(this); }
    public void AddFollower(int amount = 1) { followerCount = Mathf.Min(followerCount + amount, maxFollowers); UpdateSize(); }
    public void RemoveFollower(int amount = 1) { followerCount = Mathf.Max(followerCount - amount, 0); UpdateSize(); }
    public int GetPowerLevel() => level + followerCount;
    ...
    IEnumerator PoisonRoutine(int damagePerTick, float duration, float tickRate)
    {
        ...
        while (timer < duration && !isDead)
        {
            ...
            if (spriteRenderer != null) StartCoroutine(FlashColor(poisonColor, hurtFlashTime));
            if (health <= 0)
            {
                Die();
                break;
    ...
    void ApplyKnockbackFromNearestEnemy()
    {
        ...
        Vector2 direction = ((Vector2)transform.position - (Vector2)nearestEnemy.transform.position).normalized;
    ...
    IEnumerator FlashColor(Color color, float time)
    {
        ...
        if (spriteRenderer != null) spriteRenderer.color = originalColor == default ? before : originalColor;
    }
    ...
    void Die()
    {
        ...
        PlayerDeathWatcher watcher = GetComponent<PlayerDeathWatcher>();
        if (watcher != null) watcher.HandleDeathNow("GobboController.Die");
        if (deathSplatPrefab != null) Instantiate(deathSplatPrefab, transform.position, Quaternion.identity);
        gameObject.SetActive(false);
    }

    int GetShownSpores()
    {
        if (sporeInventory != null) return sporeInventory.spores;
        return sporeCount;
    }
    ...
    void OnGUI()
    {
        ...
        GUI.Label(new Rect(20, 140, 250, 20), "Followers: " + followerCount);
        GUI.Label(new Rect(20, 160, 250, 20), "Spores: " + GetShownSpores());
        GUI.Label(new Rect(20, 180, 250, 20), "Follower Mode: " + (followersFollowing ? "Follow" : "Stay"));
        GUI.Label(new Rect(20, 200, 250, 20), "Combat Mode: " + (followersAggressive ? "Bite" : "Passive"));
    }

    void OnDrawGizmos()
    {
        Vector2 direction = aimDirection == Vector2.zero ? Vector2.down : aimDirection.normalized;
