...
public static class GobboSaveDataExtensions
{
    ...
    public static void EnsureLeaderIdentity(this GobboSaveData data, string preferredName)
    {
        if (data == null) return;
        data.isLeader = true;
        if (!string.IsNullOrWhiteSpace(preferredName) && (string.IsNullOrWhiteSpace(data.displayName) || data.displayName == "Gobbo"))
            data.displayName = preferredName.Trim();
        data.EnsureRuntimeDefaults();
        if (data.maxHealth < 1) data.maxHealth = 100;
        if (data.health < 1 || data.health > data.maxHealth) data.health = data.maxHealth;
        if (data.moveSpeed <= 0f) data.moveSpeed = 5f;
    }

    public static GobboUnitSaveData ToUnitSave(this GobboSaveData data)
    {
        if (data == null) return new GobboUnitSaveData { isLeader = true };
    ...
    public static GobboSaveData ToLeaderSave(this GobboUnitSaveData unit)
    {
        ...
        if (unit != null) unit.CopyInto(leader);
