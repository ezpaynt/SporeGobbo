...
public class GobboCardDatabase : MonoBehaviour
{
    ...
    public List<GobboCard> GetChoicesForPlayer(GobboController gobbo, GobboCardContext context, int count, List<string> excludeIds = null)
    {
        ...
        {
            if (card == null || !card.CanAppearForPlayer(gobbo, context))
                continue;
    ...
    public List<GobboCard> GetChoicesForBuddy(BuddyData buddy, GobboCardContext context, int count, List<string> excludeIds = null)
    {
        List<GobboCard> available = new List<GobboCard>();

        foreach (GobboCard card in cards)
        {
            if (card == null || !card.CanAppearForBuddy(buddy, context))
                continue;
    ...
    void CreateDefaultsIfEmpty()
    {
        ...
        AddTypeChoice(BuddyType.Fast, "Fast Gobbo", "Become a twitchy little sprint freak. +speed, faster attacks, less health.", 2, -10, 0.9f, -0.08f);
        ...
        AddTypeChoice(BuddyType.Tank, "Tank Gobbo", "Become a walking wall. +defense and health, slower movement.", 25, 2, -0.35f, 0.05f);
        ...
        AddTypeChoice(BuddyType.Fungal, "Fungal Gobbo", "Become a weird bloom-body. Unlocks the first healing path later.", 10, 0, -0.05f, 0f, false, 0f, 0, 0, true);
        ...
        cards.Add(new GobboCard { cardId = "mushroom_gut", cardName = "Mushroom Gut", description = "+25 Max Health. Bigger, harder to squish.", maxHealthBonus = 25 });
        cards.Add(new GobboCard { cardId = "tunnel_rat", cardName = "Tunnel Rat", description = "+1 Dig Power, +0.25 Dig Radius, -0.3 Move Speed.", digPowerBonus = 1, attackBonus = 0, moveSpeedBonus = -0.3f });
        ...
        cards.Add(new GobboCard { cardId = "spore_mend", cardName = "Spore Mend", description = "Unlock R self-heal. Flat heal on cooldown.", unlockSporeMend = true, minLevel = 6, playerAllowed = true, buddyAllowed = false });
        ...
        AddClassCard(BuddyType.Thrower, "weird_arm", "Weird Arm", "+0.35 Attack Range now. Projectile range later.", attackRangeBonus: 0.35f);
        ...
        AddClassCard(BuddyType.Explosive, "pop_belly", "Pop Belly", "+0.2 Attack Radius. Future blast radius hook.", attackRadiusBonus: 0.2f);
